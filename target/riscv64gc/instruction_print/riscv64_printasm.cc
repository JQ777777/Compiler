...
template <>void RiscV64Printer::printAsm<MachineAddiwInstruction*>(MachineAddiwInstruction* ins) {
    // 输出 ADDIW 指令的汇编代码
    s << "addiw\t";
    // 假设 GetDst() 返回的是 Register 对象，并且有 getName() 方法
    s << ins->GetDst()->reg_no << ", ";
    // 假设 GetSrc() 返回的对象有 toString 方法
    s << ins->GetSrc()->toString() << ", #" << ins->GetImm();
    s << "\n"; // 根据需要决定是否换行
}
...
template <> void RiscV64Printer::printAsm<MachineBaseInstruction *>(MachineBaseInstruction *ins) {
    if (ins->arch == MachineBaseInstruction::RiscV) {
        ...
        return;
    } else if (ins->arch == MachineBaseInstruction::ADDIW) {
        printAsm<MachineAddiwInstruction *>((MachineAddiwInstruction *)ins);
        return;
    } else {
        ...
        for (auto block : func->blocks) {
            ...
            for (auto ins : *block) {
                if (ins->arch == MachineBaseInstruction::RiscV) {
                    ...
                    s << "\t";
                    printAsm((MachineAddiwInstruction *)ins);
                    s << "\n";
                } else {
