...
public:
    ...
    Operand ptr;  //Operand 声明指向 BasicOperand 对象的指针
...
/*struct FunctionInfo {
    std::vector<Type> paramTypes; // 函数参数类型列表
...
public:
    ...
    Decl decl;
    // construction
    for_stmt(Decl a, Expression c, Stmt b,Stmt d) : decl(a),Cond(c), body1(b) ,body2(d){}
    void codeIR();
    void TypeCheck();
    void printAST(std::ostream &s, int pad);
};*/
...
public:
    ...
    Stmt latch;
    Decl decl;
    for_stmt(Decl d,Expression c, Stmt l,Stmt b) : decl(d), Cond(c), latch(l),body(b) {}
    void codeIR();
    void TypeCheck();
    void printAST(std::ostream &s, int pad);
};
