...
enum {
    ...
    RISCV_MUL, //32位整数乘法
    RISCV_MULH, //32位有符号整数乘法，高32位结果
    RISCV_MULHSU, //32位混合符号乘法，高32位结果
    RISCV_MULHU, //32位无符号整数乘法，高32位结果
    RISCV_DIV,
    RISCV_DIVU,
    RISCV_REM,
    RISCV_REMU,
    RISCV_MULW, //64位有符号整数乘法（RV64I），结果截断为32位
    RISCV_DIVW,
    RISCV_REMW,
    RISCV_REMUW,
...
struct RiscVLabel {
    ...
    // 添加一些你想用的构造函数
    RiscVLabel(int jmp) : jmp_label_id(jmp), is_data_address(false), name(){}
    RiscVLabel(int jmp, int seq) : jmp_label_id(jmp), seq_label_id(seq), name() {}
};
...
public:
    ...
    // 公共类型检查函数
    void CheckTypeCompatibility(Register dst, MachineDataType type) {
        Assert(dst.type == type);  // 检查寄存器类型和目标类型是否一致
    }

        // 构造 MachineCopyInstruction 的通用函数
    MachineCopyInstruction* ConstructCopyInstruction(MachineBaseOperand* srcOperand, MachineBaseOperand* dstOperand, MachineDataType type) {
        MachineCopyInstruction *ret = new MachineCopyInstruction(srcOperand, dstOperand, type);
        ret->SetNoSchedule(no_schedule);  // 设置不调度
        return ret;
    }

    // 构造寄存器到寄存器的拷贝
    MachineCopyInstruction* ConstructCopyReg(Register dst, Register src, MachineDataType type) {
        CheckTypeCompatibility(dst, src.type);  // 确保类型一致
        CheckTypeCompatibility(dst, type);      // 确保目标类型一致

        return ConstructCopyInstruction(new MachineRegister(src), new MachineRegister(dst), type);
    }

    // 构造寄存器到立即数的拷贝，整数类型
    MachineCopyInstruction* ConstructCopyRegImmI(Register dst, int src, MachineDataType type) {
        Assert(type.data_type == MachineDataType::INT);  // 确保类型是整数

        CheckTypeCompatibility(dst, type);  // 检查寄存器类型与目标类型是否一致

        return ConstructCopyInstruction(new MachineImmediateInt(src), new MachineRegister(dst), type);
    }

    // 构造寄存器到立即数的拷贝，浮点数类型
    MachineCopyInstruction* ConstructCopyRegImmF(Register dst, float src, MachineDataType type) {
        Assert(type.data_type == MachineDataType::FLOAT);  // 确保类型是浮点数

        CheckTypeCompatibility(dst, type);  // 检查寄存器类型与目标类型是否一致

        return ConstructCopyInstruction(new MachineImmediateFloat(src), new MachineRegister(dst), type);
    }

    // 构造寄存器到立即数的拷贝，双精度浮点数类型
    MachineCopyInstruction* ConstructCopyRegImmF64(Register dst, double src, MachineDataType type) {
        Assert(type.data_type == MachineDataType::FLOAT);  // 确保类型是浮点数

        CheckTypeCompatibility(dst, type);  // 检查寄存器类型与目标类型是否一致

        return ConstructCopyInstruction(new MachineImmediateDouble(src), new MachineRegister(dst), type);
    }
...
public:
    // TODO: add your own members here
    void addlist_alloca_ins(RiscV64Instruction *ins) { allocalist.push_back(ins); }
};
...
private:
    ...
    void GenerateCopyToStackCode(std::list<MachineBaseInstruction *>::iterator &it, int raw_stk_offset,
                                             Register reg, MachineDataType type);
    void GenerateCopyFromStackCode(std::list<MachineBaseInstruction *>::iterator &it, int raw_stk_offset,
                                               Register reg, MachineDataType type);
};
