...
protected:
    LLVMIROpcode opcode;
    LLVMType type;
    std::unordered_set<BasicInstruction*> useList;  // 使用列表
    std::unordered_set<Operand*> useOpList;  // 操作数列表

public:
    int GetOpcode() { return opcode; }    // one solution: convert to pointer of subclasses
    // 获取所有使用该指令的地方
    const std::unordered_set<BasicInstruction*>& uses() {
        return useList;
    }
    const std::unordered_set<Operand*>& useOps() {
        return useOpList;
    }


    virtual std::vector<RegOperand*> GetRegOperands()= 0;
    // 获取指令中所有使用的寄存器
    /*
    std::vector<RegOperand*> GetRegOperands() const {
        std::vector<RegOperand*> regOperands;
        for (auto use : useOpList) {
            if ((*use)->GetOperandType() == BasicOperand::REG) {
                RegOperand* regOp = dynamic_cast<RegOperand*>(*use);
                if (regOp != nullptr) {
                    regOperands.push_back(regOp);  // 获取寄存器操作数
                }
            }
        }
        return regOperands;  // 返回寄存器操作数的列表
    }
    */


    // 获取指令的类型
    LLVMType GetType() { return type; }

    // 检查是否是指令为 Alloca
    bool isAlloca() { return GetOpcode() == ALLOCA; }
    virtual void PrintIR(std::ostream &s) = 0;
    virtual void Replace_RegMap(const std::map<int, int> &ReMap)= 0;
...
public:
    ...
    void Replace_RegMap(const std::map<int, int> &ReMap);

// 获取 Load 指令中的寄存器操作数
    virtual std::vector<RegOperand*> GetRegOperands() {
        std::vector<RegOperand*> regOperands {};  // 获取操作数中的寄存器
        if (pointer && pointer->GetOperandType()==BasicOperand::REG) {
            RegOperand* regOp = dynamic_cast<RegOperand*>(pointer);
            if (regOp != nullptr) {
                regOperands.push_back(regOp);  // 将结果寄存器也加入寄存器列表
            }
        }
        return regOperands;
    }
...
public:
    ...
    virtual std::vector<RegOperand*> GetRegOperands() {
        std::vector<RegOperand*> regOperands {};  // 获取操作数中的寄存器
        if (value && value->GetOperandType()==BasicOperand::REG) {
            RegOperand* regOp = dynamic_cast<RegOperand*>(value);
            if (regOp != nullptr) {
                regOperands.push_back(regOp);  // 将值寄存器也加入寄存器列表
            }
        }
...
public:
    ...
    //LLVMIROpcode GetOpcode() { return this->opcode;}
    ...
    virtual void PrintIR(std::ostream &s);
    void Replace_RegMap(const std::map<int, int> &ReMap);
    // 获取 Arithmetic 指令中的寄存器操作数
    virtual std::vector<RegOperand*> GetRegOperands() {
        std::vector<RegOperand*> regOperands {};  // 获取操作数中的寄存器

        // 检查 op1
        if (op1 && op1->GetOperandType() == BasicOperand::REG) {
            RegOperand* regOp = dynamic_cast<RegOperand*>(op1);
            if (regOp != nullptr) {
                regOperands.push_back(regOp);  // 将寄存器 op1 加入寄存器列表
            }
        }

        // 检查 op2
        if (op2 && op2->GetOperandType() == BasicOperand::REG) {
            RegOperand* regOp = dynamic_cast<RegOperand*>(op2);
            if (regOp != nullptr) {
                regOperands.push_back(regOp);  // 将寄存器 op2 加入寄存器列表
            }
        }
...
public:
    ...
    std::vector<Operand> GetOperands() {
        ...
        for (const auto& pair : phi_list) {
            operands.push_back(pair.second);  // 获取每个 pair 的第二个元素，即操作数
        }
        return operands;
    }
    ...
    virtual std::vector<RegOperand*> GetRegOperands() {
        ...
        // 获取 Phi 列表中的寄存器操作数
        for (const auto& [Labelreg, Regop] : phi_list) {
            if (Regop && Regop->GetOperandType() == BasicOperand::REG) {
                RegOperand* regOp = dynamic_cast<RegOperand*>(Regop);
                if (regOp != nullptr) {
                    regOperands.push_back(regOp);  // 将寄存器加入寄存器列表
                }
            }
        }
...
public:
    ...
    virtual std::vector<RegOperand*> GetRegOperands() {
        ...
        // 检查 cond 寄存器
        if (cond && cond->GetOperandType() == BasicOperand::REG) {
            regOperands.push_back(dynamic_cast<RegOperand*>(cond));
        }

        // trueLabel 和 falseLabel 是标签操作数，通常它们不会是寄存器类型
        // 这里只处理寄存器操作数，不处理标签
...
public:
    ...
    virtual std::vector<RegOperand*> GetRegOperands() {
        ...
        // 检查 args 中的操作数
        for (auto& arg : args) {
            if (arg.second && arg.second->GetOperandType() == BasicOperand::REG) {
                regOperands.push_back(dynamic_cast<RegOperand*>(arg.second));
            }
        }
...
public:
    ...
    virtual std::vector<RegOperand*> GetRegOperands() {
        ...
        if (ret_val && ret_val->GetOperandType() == BasicOperand::REG) {
            regOperands.push_back(dynamic_cast<RegOperand*>(ret_val));
        }
...
public:
    ...
    virtual std::vector<RegOperand*> GetRegOperands() {
        ...
        if (ptrval && ptrval->GetOperandType() == BasicOperand::REG) {
            regOperands.push_back(dynamic_cast<RegOperand*>(ptrval));
        }
        // 检查所有索引操作数中的寄存器
        for (const auto& idx : indexes) {
            if (idx && idx->GetOperandType() == BasicOperand::REG) {
                regOperands.push_back(dynamic_cast<RegOperand*>(idx));
            }
        }
        return regOperands;
    }
...
public:
    ...
    virtual std::vector<RegOperand*> GetRegOperands() {
        ...
        // 检查 value 寄存器
        if (value && value->GetOperandType() == BasicOperand::REG) {
            regOperands.push_back(dynamic_cast<RegOperand*>(value));
        }
